...
static void *slob_page_alloc(struct slob_page *sp, size_t size, int align)
{
	...
	slob_t *best_prev = NULL, *best_cur = NULL ;
	...
	for (prev = NULL, cur = sp->free; ; prev = cur, cur = slob_next(cur)) {
		...
		// first entry or better block found
...
static int best_fit_check(struct slob_page *sp, size_t size, int align)
{
	...
	for (prev = NULL, cur = sp->free; ; prev = cur, cur = slob_next(cur))
	{
		...
		// if first available block or better fit block found
...
static void *slob_alloc(size_t size, gfp_t gfp, int align, int node)
{
	...
	list_for_each_entry(sp, slob_list, list) 
	{
		...
		if (temp == 0)	//best case found
		{
			best = sp ;
			best_fit = 0 ;
			break ;
		}
		else if (temp > 0 && (best == NULL || temp < best_fit))
		{
			best = sp ;
			best_fit = temp ;
		}
	...
	if (!b) {
		...
		{
			claimsize[i] = size ;
			freesize[i] = (sum * SLOB_UNIT) - SLOB_UNIT + 1  ;
			i = (i + 1) % 100 ;
		}
...
asmlinkage long sys_get_slob_amt_claimed(void)
{
	...
	if (x ==0) printk("NO FRAGMENTATION\n") ;
	else
	{
		for( j=0; j <= x; j++) avg += claimsize[j] ;
		avg = avg / x ;
		printk("Average claimed amount %zu\n", avg) ;
	}
	return 0 ;	
}
...
asmlinkage long sys_get_slob_amt_free(void)
{
	...
	{
		for( j=0; j <= x; j++) avg += freesize[j] ;
		avg = avg / x ;
		printk("Average free amount for %zu\n", avg) ;
	}
	return 0 ;	
}
